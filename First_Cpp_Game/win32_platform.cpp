...
{	
	...
	switch (uMsg)
	{
		...
		case WM_SIZE: {
			...
			int size = render_state.width * render_state.height * sizeof(unsigned int);			//Size of Current render_buffer

			if (render_state.memory) VirtualFree(render_state.memory, 0, MEM_RELEASE);          //Free Buffer_Memory if Previously Allocated 
			render_state.memory = VirtualAlloc(0, size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE); // Assign Buffer_Memory 
