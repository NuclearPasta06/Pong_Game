...
simulate_player(float* p, float* dp, float ddp, float dt) {
	...
	ddp -= *dp * 10.f;	//Simulate Friction

	*p = *p + *dp * dt + ddp * dt * dt * .5f;  //Update Postion with Accelaration
	*dp = *dp + ddp * dt;						//Update Speed
...
simulate(Input *input,float dt)
{
	...
	if (current_gamemode == GM_GAMEPLAY) {
		...
		draw_rect(80, player_1_p, 2.5, 12, 0x00f500);
		draw_rect(-80, player_2_p, 2.5, 12, 0x0000f5);

		draw_text("MOVEMENT KEYS", -73, 40, .5, 0xffffff);
		draw_text("UP/W DOWN/S", -73, 30, .5, 0xffffff);
		...
		if (hot_button == 0) {
			...
			draw_text("MULTIPLAYER", 20, 0, 1, 0xaaaaaa);
		}
		else {
			draw_text("SINGLE PLAYER", -80, 0, 1, 0xaaaaaa);
			draw_text("MULTIPLAYER", 20, 0, 1, 0xff0000);
		}

		draw_text("PONG", -19, 30, 2, 0xffffff);
		draw_text("SELECT/ENTER", -19, -30, 0.5, 0xffffff);
	}
}